...
export function DatePicker({ value, onChange }){
    ...
    return (
        ...
        <div className="date-picker-container">
            <button className="date-picker-button" onClick={() => setDatePickerModalOpen(o => !o)}>
                 {value == null ? "Select a Date" : format(value, "MMM do, yyyy")} 
            </button>

            {isDatePickerModalOpen && <DatePickerModal  dateChange={onChange} value={value}/>}
        </div>
        </>
    )
}

function DatePickerModal({ value, dateChange}){
